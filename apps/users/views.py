...
class LoginView(View):
    ...
    def post(self, request):
        ...
        if login_form.is_valid():
            # 取不到时为空，username，password为前端页面的name值!!!注意这里的POST不要写成了Post
            ...
            # 如果不是null说明验证成功
            if user is not None:
                # login_in两参数：request，user
                # 实际是对request写了一部分东西进去，然后再render的时候
                # request是要render回去的，这些信息也就随着返回浏览器，完成登录
                login(request, user)
                # 跳转到首页user request会被带回到首页
